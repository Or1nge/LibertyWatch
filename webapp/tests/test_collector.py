...
def test_no_push_output_is_atomic_and_cli_testable(tmp_path: Path) -> None:
    ...
    v2_output = tmp_path / "v2_latest_snapshot.json"
    exit_code = main(
        [
            "--config",
            str(config_path),
    ...
    assert json.loads(v2_output.read_text(encoding="utf-8")) == payload
    assert stat.S_IMODE(v2_output.stat().st_mode) == 0o640
