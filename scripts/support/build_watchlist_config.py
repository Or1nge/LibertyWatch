...
def _yield_bases() -> dict[str, dict[str, Any]]:
    ...
    for row in _read_rows(ANNUAL_YIELD_CSV):
    ...
    for row in _read_rows(ARCHIVED_ANNUAL_YIELD_CSV):
        if row["市场"] != "A股":
            continue
    ...
    for row in _read_rows(ARCHIVED_EXTENSION_CSV):
...
def build_watchlist() -> dict[str, Any]:
    issuers = {
        row["company_name"]: row["issuer_id"] for row in _read_rows(ISSUER_CSV)
    }
